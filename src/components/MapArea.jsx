...
export default function MapArea({
  ...
  const METRICS = [
    { key: 'targetGenderAge', label: `${targetAge} ${targetGender} 비율`, unit: '%' },
  ...
  const intensities = useMemo(() => {
    const values = dongs.map((d) => metricValue(d, metric, targetAge, targetGender))
  ...
  const ranked = useMemo(
    () =>
      [...dongs]
        .map((d) => ({ ...d, value: metricValue(d, metric, targetAge, targetGender) }))
    ...
    <div className="relative h-full w-full">
              ...
              >
                <div className="text-xs">
                  ...
                  <div className="text-slate-700">
                    {metricInfo.label}{' '}
                    <b>
                      {formatValue(metric, metricValue(d, metric, targetAge, targetGender))}
                      {metricInfo.unit}
                    </b>
                  </div>
          ...
          <div className="space-y-1">
            {top5.map((d, idx) => (
              ...
              >
                <span>
                  {idx + 1}. {d.dong}
                  <span className="ml-1 text-[10px] text-slate-500">({d.gu})</span>
                </span>
      ...
      <div className="pointer-events-none absolute bottom-4 left-1/2 z-[400] -translate-x-1/2">
        <div className="pointer-events-auto rounded-full border border-white/10 bg-slate-900/85 px-5 py-2 text-[11px] text-slate-300 shadow-xl backdrop-blur-md">
          💡 색이 짙은 동을 클릭하면 실측 인구·연령·외국인·세대 통계 리포트가 열립니다
          {selectedDong && (
